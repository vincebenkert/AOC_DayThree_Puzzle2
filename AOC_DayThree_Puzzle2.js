...
var numberOfSteps = 35000;
var testing = 0;
var splitArrayOne = new Array();
var splitArrayTwo = new Array();


for(var h = 0; h < arrayOneLength; h++){
	splitArrayOne = wireOneCoords[h].split(',');
	for(var d = 0; d < arrayTwoLength; d++){
  	splitArrayTwo = wireTwoCoords[d].split(',');
    
    	if(splitArrayOne[0] == splitArrayTwo[0] && splitArrayOne[1] == splitArrayTwo[1]){
      	testing = Number(splitArrayOne[2]) + Number(splitArrayTwo[2]);
        if(testing < numberOfSteps){
        numberOfSteps = testing;
        console.log("Current Minmium distance from crosswires is... " + numberOfSteps);
        }
        	
      
      }
//This won't work anymore because I changed the array to include steps which is always different....
...
console.log("Hey this took awhile...  but the program is completed....");
